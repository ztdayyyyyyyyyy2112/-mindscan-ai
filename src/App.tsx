...
const GaugeChart = ({ level, confidence, t, isDarkMode }: { level: string, confidence: number, t?: (key: string) => string, isDarkMode?: boolean }) => {
  ...
  let rotation = 0;
  const c = Math.max(0, Math.min(1, confidence || 0)); // clamp to [0,1], handle NaN/undefined
  if (level === 'Low') {
    rotation = -90 + (1 - c) * 60;
  ...
  // Always use translated text when t() is available
  const levelText = t
    ? t(`results.${level.toLowerCase()}`)
    : (level === 'Low' ? 'LOW' : level === 'Medium' ? 'MEDIUM' : 'HIGH');
  const levelColor = level === 'Low' ? '#6ee7b7' : level === 'Medium' ? '#fcd34d' : '#fb7185';
  const confidencePercent = Math.round(c * 100);
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-64 h-36 overflow-hidden">
        <svg viewBox="0 0 200 110" className="w-full h-full">
          ...
          <text x="30" y="108" textAnchor="middle" fontSize="8" fill={isDarkMode ? '#9ca3af' : '#6b7280'} fontWeight="600">
            {t ? t('results.low') : 'LOW'}
          </text>
          <text x="100" y="108" textAnchor="middle" fontSize="8" fill={isDarkMode ? '#9ca3af' : '#6b7280'} fontWeight="600">
            {t ? t('results.medium') : 'MED'}
          </text>
          <text x="170" y="108" textAnchor="middle" fontSize="8" fill={isDarkMode ? '#9ca3af' : '#6b7280'} fontWeight="600">
            {t ? t('results.high') : 'HIGH'}
          </text>
      ...
      <div className="text-center mt-2">
        <div className={`text-lg font-bold mb-1 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>
          {t ? t('results.gaugeTitle') : 'Mức độ Stress Hiện tại:'} <span style={{ color: levelColor }}>{levelText}</span>
        </div>
        <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          {t ? t('results.gaugeConf') : 'Độ Tự tin (Confidence):'} {confidencePercent}%
        </div>
      </div>
    </div>
  );
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <CustomSlider min={0} max={10} step={1} value={formData.anxiety_level} onChange={(v) => handleInputChange('anxiety_level', v)} ariaLabel="Anxiety" />
                <div className={`flex justify-between text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><span>0</span><span>10</span></div>
              </div>
              <div className="space-y-4">
                <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-800'}`}>{t('questions.q8')}</label>
                <CustomSlider min={0} max={10} step={1} value={formData.depression} onChange={(v) => handleInputChange('depression', v)} ariaLabel="Depression" />
                <div className={`flex justify-between text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><span>0</span><span>10</span></div>
              </div>
              <div className="space-y-4">
                <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-800'}`}>{t('questions.q9')}</label>
                <CustomSlider min={0} max={10} step={1} value={formData.self_esteem} onChange={(v) => handleInputChange('self_esteem', v)} ariaLabel="Self Esteem" />
                <div className={`flex justify-between text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}><span>0</span><span>10</span></div>
              </div>
              <div className="space-y-4">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md" onClick={() => setShowPrivacyModal(false)}>
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-[#0b132b]'}`}>{t('modals.privacyTitle')}</h2>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md" onClick={() => setShowEthicsModal(false)}>
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-[#0b132b]'}`}>{t('modals.ethicsTitle')}</h2>
              </div>
              <div className={`space-y-5 text-sm leading-relaxed ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                <div>
                  ...
                  <div className="text-left w-full max-w-4xl mx-auto">
                    ...
                    {sessionHistory.length > 0 && (
                      <div className="mb-12">
                        ...
                        <div className="space-y-4">
                          {sessionHistory.slice(-5).map((session, idx) => (
                        ...
                        {sessionHistory.length > 5 && (
                          <div className="text-center mt-4">
                            <button
                              onClick={() => {
                                const el = document.getElementById('mindscan-full-log');
                                if (el) el.classList.toggle('hidden');
                              }}
                              className={`inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold transition-all ${isDarkMode ? 'bg-white/10 border border-white/20 text-gray-300 hover:bg-white/20' : 'bg-gray-100 border border-gray-200 text-gray-700 hover:bg-gray-200'}`}
                            >
                              <BarChart2 className="w-4 h-4" />
                              {t('results.viewFullLog')}
                            </button>
                          </div>
                        )}
                        <div id="mindscan-full-log" className="hidden mt-4 space-y-4">
                          {sessionHistory.slice(0, -5).map((session, idx) => (
                            <div key={`full-history-${session.date}-${idx}`} className={`flex items-center gap-4 backdrop-blur-sm border p-4 rounded-2xl ${isDarkMode ? 'bg-[#0b132b]/50 border-white/10' : 'bg-white/20 border-white/30'}`}>
                              <div className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap ${
                                session.level === 'High' ? 'bg-red-100 text-red-700' :
                                session.level === 'Medium' ? 'bg-yellow-100 text-yellow-700' :
                                'bg-green-100 text-green-700'
